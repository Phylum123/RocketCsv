    ...
    public partial class OrderedDictionary<TKey, TValue> : IDictionary<TKey, TValue>, IReadOnlyDictionary<TKey, TValue>, IList<KeyValuePair<TKey, TValue>>, IReadOnlyList<KeyValuePair<TKey, TValue>>
    {
        ...
        public TValue this[TKey key]
        {
            get
            {
                ...
                if (index < 0)
                {
                    throw new KeyNotFoundException(string.Format("Key '{0}' was not found", key.ToString()));
                }
        ...
        public void Move(int fromIndex, int toIndex)
        {
            ...
            {
                throw new ArgumentOutOfRangeException(nameof(fromIndex), "From index out of range.");
            }
            if ((uint)toIndex >= (uint)Count)
            {
                throw new ArgumentOutOfRangeException(nameof(toIndex), "To index out of range");
            }
        ...
        KeyValuePair<TKey, TValue> IList<KeyValuePair<TKey, TValue>>.this[int index]
        {
            ...
            set
            {
                ...
                {
                    throw new ArgumentException(string.Format("Cannot add duplicate key '{0}'", key.ToString()));
                }
            }
        }
        ...
        void ICollection<KeyValuePair<TKey, TValue>>.CopyTo(KeyValuePair<TKey, TValue>[] array, int arrayIndex)
        {
            ...
            {
                throw new ArgumentOutOfRangeException(nameof(arrayIndex), "Must be 0 or greater.");
            }
            int count = Count;
            if (array.Length - arrayIndex < count)
            {
                throw new ArgumentException("Nout enough room in array.");
            }
            ...
            else
            {
                ...
                while (true)
                {
                    ...
                    if (collisionCount >= entries.Length)
                    {
                        // The chain of entries forms a loop; which means a concurrent update has happened.
                        // Break out of the loop and throw, rather than looping forever.
                        throw new InvalidOperationException("Current operation not supported!");
                    }
                    ++collisionCount;
                }
            }
